...
test.describe('Resume Upload Flow', () => {
  ...
  test('should enable submit button when both file and description are provided', async ({ page }) => {
  ...
  test('should handle async job processing with polling', async ({ page }) => {
    ...
    await expect(page.getByText(/analysis complete/i)).toBeVisible({ timeout: 15_000 });
