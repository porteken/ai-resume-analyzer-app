...
export const useResumeAnalysis = (): UseResumeAnalysisReturn => {
  ...
  const submitAnalysis = useCallback(async (file: File | null, jobDescription: string) => {
    const validationError = validateFile(file) || validateJobDescription(jobDescription);
    if (validationError) {
      setResult(null);
      setStatusMessage("");
    ...
    try {
      setStatusMessage("Uploading Resume...");
      const uploadData = await uploadResume(file, jobDescription.trim(), setStatusMessage, {
